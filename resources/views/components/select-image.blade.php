      ...
      <main class="modal__content" id="{{$modal}}-content">
        <div class="flex flex-wrap">
                    ...
                    <div class="w-1/2 p-2 mb:p-4" >
                      ...
                      data-file="{{$image->filename}}" data-path="{{asset('storage/product/')}}"
                      data-modal="{{$modal}}"
                      src="{{asset('storage/product/'.$image->filename)}}" >
...
<div class="w-1/4">
<img id="{{$name}}_thumbnail" @if($cImage) src="{{asset('storage/product/'. $cImage)}}" @else src='' @endif>
</div>
</div>
